...
export class Register2Component {
  ...
  constructor(private router: Router, private authService: AuthService, private snackBar: MatSnackBar, private cookieService: CookieService) {}
  ...
  comprobarCodigo(codigo = 1) {
    ...
    this.authService.comprobarCodigo(body).subscribe((res) => {
      if(res.resultado == "Correcto"){
        this.router.navigate(['/auth/register3']); 
      }else{
